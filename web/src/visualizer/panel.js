...
export function openPanel(node) {
  ...
  if (readOnly) {
    html += `<div class="desc-block">Static preview mode: script graph browsing is available, but editing and scene actions are disabled.</div>`;
  }
  ...
  for (let vi = 0; vi < exportVars.length; vi++) {
    ...
    html += `<span class="${editableClass} var-name" ${editableAttr} data-field="name" data-original="${esc(v.name)}">${esc(v.name)}</span>`;
    html += `<span class="ret">:</span> `;
    html += `<span class="tp ${editableClass} var-type" ${editableAttr} data-field="type" data-placeholder="Type" data-original="${esc(v.type || '')}">${esc(v.type || '')}</span>`;
    html += ` <span class="ret">=</span> `;
    html += `<span class="num ${editableClass} var-default" ${editableAttr} data-field="default" data-placeholder="value" data-original="${esc(v.default || '')}">${esc(v.default || '')}</span>`;
    if (!readOnly) {
  ...
  for (let vi = 0; vi < regularVars.length; vi++) {
    ...
    if (v.onready) {
      html += readOnly
        ? `<span class="onready-badge">@onready</span>`
        : `<span class="onready-badge" onclick="toggleOnready(${vi}, false)" title="Click to toggle @onready">@onready</span>`;
    }
    html += `<span class="kw">var</span> `;
    html += `<span class="${editableClass} var-name" ${editableAttr} data-field="name" data-original="${esc(v.name)}">${esc(v.name)}</span>`;
    html += `<span class="ret">:</span> `;
  ...
  if ((node.functions || []).length > 0) {
    ...
    for (let fi = 0; fi < node.functions.length; fi++) {
      ...
      if (!readOnly) html += `<button class="delete" onclick="event.stopPropagation();showDeleteUsages(${fi}, false, 'function')" title="Delete function" style="opacity:0">×</button>`;
      html += `</span>`;
      html += `</li>`;
  ...
  for (let si = 0; si < signalsList.length; si++) {
    ...
    html += `<span class="sig ${editableClass} signal-name" ${editableAttr} data-field="name" data-original="${esc(sigName)}">${esc(sigName)}</span>`;
    html += `<span class="param">(</span>`;
    html += `<span class="${editableClass} signal-params" ${editableAttr} data-field="params" data-placeholder="params" data-original="${esc(sigParams)}">${esc(sigParams)}</span>`;
    html += `<span class="param">)</span>`;
    if (!readOnly) {
      html += `<span class="item-actions">`;
...
window.toggleFunc = function (fi) {
  ...
  viewer.innerHTML = `
    ...
    <div class="func-viewer-code">
      <div class="code-editor-container" id="${editorId}">
        ...
        <textarea class="code-editor-textarea" id="textarea-${fi}" spellcheck="false" ${readOnly ? 'readonly' : ''}></textarea>
      </div>
    </div>
    <div class="func-viewer-footer">
      <span class="status ${readOnly ? 'visible' : ''}" id="status-${fi}">${readOnly ? 'Read-only preview' : 'Modified'}</span>
      <span style="display:flex;gap:8px;align-items:center">
        <span style="opacity:0.6">${readOnly ? 'Editing disabled in static preview' : 'Ctrl+S to save'}</span>
        ${readOnly ? '' : `<button class="save-btn" id="save-btn-${fi}" onclick="saveFunction(${fi})">Save</button>`}
      </span>
    </div>
  `;
  ...
  textarea.addEventListener('input', () => {
    ...
    statusEl.classList.toggle('visible', readOnly || modified);
    if (saveBtn) saveBtn.classList.toggle('active', modified);
...
window.saveFunction = async function (fi) {
  if (isStaticPreview()) {
    alert('Static preview mode is read-only. Re-export from the plugin after making changes in Godot.');
    return;
  }
